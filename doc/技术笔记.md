...
在一个历史提交中错误提交文件，但是许久后发现，现在需要去除该文件影响。
...
一、首先配置github的ssh，拥有访问权限

```shell
ssh-keygen [ -t dsa | ecdsa | ecdsa-sk | ed25519 | ed25519-sk | rsa] -C "your email"
```

二、登录github，将.ssh/key.pub中内容添加到github密钥设置中

三、修改本地.ssh/config配置文件，添加如下内容
...
四、注意需要再次确认是否设置git配置，git config global...
...
九、修改完冲突后再次正常提交即可。奇怪的分支操作具体参见上一小节中的远程库操作
